...
def train():
    ...
    if args.dataset_type == 'synth360':
        ...
        # poses = poses[:,:3,:4]
        # print(poses.shape)
        print('Loaded 360', "train_len:",train_dataload.__len__(),"test_len:",test_dataload.__len__())
        # if not isinstance(i_test, list):
        #     i_test = [i_test]
        ...
        #     i_test = np.arange(images.shape[0])[::args.llffhold]
        # print(":",i_test)
        i_train = train_dataload.__len__()
        i_test = test_dataload.__len__()
        i_val = i_test
        # print(i_train)
        print('DEFINING BOUNDS')
        # if args.no_ndc:
    ...
    for i in trange(start, N_iters):
        for t,data in tqdm(enumerate(dataloader)):
            ...
            pose = pose[0] ; image = image[0] ; rays_o = rays_o[0] ; rays_d = rays_d[0]
